    ...
    <div class="modal-overlay" :class="{ open: visible }" @click.self="$emit('close')">
      <div class="modal">
        ...
        <div class="modal-actions">
          ...
          <button class="btn-accent" style="flex: 2" @click="save">Add Expense</button>
        </div>
      </div>
    </div>
  </Teleport>
...
const props = defineProps({ visible: Boolean, preselectedTab: { type: String, default: '' } })
const emit = defineEmits(['close'])

const store = useAppStore()

const form = reactive({ tabId: '', desc: '', amount: null, date: '', note: '' })

watch(
  () => props.visible,
  (v) => {
    if (v) {
      form.tabId = props.preselectedTab || store.tabs[0]?.id || ''
      form.desc = ''
      form.amount = null
...
async function save() {
  if (!form.desc.trim() || !form.amount || form.amount <= 0 || !form.date) {
    window.__spendlyToast?.('Please fill in description, amount and date')
    return
  }
  const exp = {
  ...
  window.__spendlyToast?.('Expense added')
  emit('close')
}
</script>
